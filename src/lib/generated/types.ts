 ...
 * @type { ProjectKind }
 */
export type ProjectKind = 'whiteboard' | 'doc' | 'todo' | 'sheet';
...
export function decodeAppMemberRow(rawInput: unknown): AppMemberRow | null {
	if (isJSON(rawInput)) {
		...
		if (decodedEmail === null || decodedIsAdmin === null || decodedCreatedAt === null) {
			return null;
		}
...
export function decodePasskeyRegisterFinishBody(
	rawInput: unknown
): PasskeyRegisterFinishBody | null {
	if (isJSON(rawInput)) {
