...
export const load: PageServerLoad = async ({ locals }) => {
	...
	if (members.error) {throw error(500, members.error.message);}
	if (invites.error) {throw error(500, invites.error.message);}
...
export const actions: Actions = {
	add: async ({ request, locals }) => {
		if (!locals.isAdmin) {return fail(403, { message: 'Forbidden' });}
		const form = await request.formData();
		const email = String(form.get('email') ?? '')
			.trim()
			.toLowerCase();
		const isAdmin = form.get('is_admin') === 'on';
		if (!email || !email.includes('@')) {return fail(400, { message: 'Invalid email' });}

		const { error: dbError } = await locals.supabase
			.from('app_members')
			.upsert(
				{ email, is_admin: isAdmin, added_by: locals.user!.id },
				{ onConflict: 'email' }
			);
	...
	createInvite: async ({ request, locals }) => {
		...
		const expiresIn = expiresInDaysRaw
			? Math.max(1, parseInt(expiresInDaysRaw, 10) || 0)
			: null;
